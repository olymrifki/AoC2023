...
def mark_map(position, maps):
    result = maps.copy()
    result[position[1]] = (
        result[position[1]][0 : position[0]]
        + "#"
        + result[position[1]][1 + position[0] :]
    )
    return result
...
def djikstra(source_position, destination_position, heuristic):
    ...
    # assert destination_position in queue
    ...
    while queue:
        ...
        # print("Len of neighbours", len(neighbours))
