...
def calculate_score(initial_beam: Beam):
    reset_scoreboard()
    beams = [initial_beam]
    while beams:
        beam = beams.pop(0)
        beams += move(beam, maps)
    return sum([sum([bool(s) for s in scoreboard[j]]) for j in range(len(scoreboard))])


all_score = []
# right direction beams:
for i in range(len(maps)):
    all_score.append(calculate_score(Beam((0, i), "right")))
# down direction beams:
for i in range(len(maps[0])):
    all_score.append(calculate_score(Beam((i, 0), "down")))

# left direction beams:
for i in range(len(maps)):
    all_score.append(calculate_score(Beam((len(maps[0]) - 1, i), "left")))
# up direction beams:
for i in range(len(maps[0])):
    all_score.append(calculate_score(Beam((i, len(maps) - 1), "up")))
print(max(all_score))
